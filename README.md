...
| **Bedrock MCPACK** | Un `.mcpack` por perfil | Color, `_normal`, `_mer` y `*.texture_set.json` | Releases oficiales Mojang o Custom ZIP/MCPACK; Patched/MBLoader, Vibrant Visuals o RTX. |
...
### Bedrock: tres perfiles de renderizado

La opción Bedrock crea un **resource pack**, no un APK. Cada textura genera color PNG, normal plana y mapa MER, donde RGB representa metalness, emissive y roughness. La interfaz genera un manifest con `min_engine_version: [1, 21, 120]` y ajusta su capability al perfil elegido, conforme a las capabilities PBR y ray tracing descritas por Microsoft. [1] [2] [3]

| Perfil, en orden de la interfaz | Manifest | Uso y aviso |
| --- | --- | --- |
| **Minecraft Patched / MBLoader** | Sin capability PBR/RTX forzada. | Exporta MCPACK para importar en un loader Android y usar con un shader compatible. Si el shader no ofrece SSR, la reflexión visible puede limitarse al cielo; el pack no añade SSR. MBLoader importa MCPACK pero funciona como instancia separada de Minecraft. [9] |
| **Vibrant Visuals** | `pbr` | Perfil PBR oficial para Vibrant Visuals. Puede haber diferencias o fallos de renderizado según versión, dispositivo y recursos o shaders activos. |
| **RTX** | `raytraced` | Perfil de ray tracing para equipos y modo RTX compatibles. Puede mostrar diferencias o fallos de renderizado y verse más oscuro según iluminación, GPU, versión y recursos activos; requiere ray tracing activado dentro de Bedrock. |

> **Límite de plataformas:** la presencia de MER y normal plana no garantiza el mismo reflejo en cada renderizador. La correspondencia exacta con texturas vanilla de Bedrock depende de que los nombres y rutas de origen coincidan con los identificadores de Bedrock; revisa el pack importado en el juego antes de usarlo en un mundo importante.
...
2. En Bedrock selecciona primero el perfil: **Minecraft Patched / MBLoader**, **Vibrant Visuals** o **RTX**. Lee el aviso del perfil antes de convertir.
3. Para Java, arrastra uno o varios ZIP/JAR. Para Bedrock, el origen predeterminado es el **Repositorio oficial**: selecciona una release estable o Preview de Mojang y pulsa **Usar esta release oficial**. La herramienta siempre señala el asset `full.zip`, que sí conserva texturas y otros binarios; nunca selecciona `min.zip`.
4. GitHub no permite que un navegador estático lea en todos los casos el ZIP de release de forma cruzada. Cuando ocurra, el botón iniciará la descarga oficial y el panel indicará cambiar a **Custom** para subir ese mismo ZIP. Custom queda reservado exclusivamente para ZIP de resource pack o MCPACK propios.
5. Selecciona un preset Glossy y revisa el preview permanente **Antes / Después**. Pulsa convertir; cada archivo produce una salida independiente: ZIP para Java o MCPACK para Bedrock.
...
Para Java, se recomienda [Complementary Reimagined][5], disponible en [Modrinth][5] y [CurseForge][6]. Para Bedrock, importa el MCPACK en el flujo que corresponda al perfil: Custom/recursos de Bedrock para Vibrant Visuals o RTX, o el importador de MCPACK del loader para Minecraft Patched / MBLoader. [9]
...
[9]: https://faizul726.github.io/blog/mb-loader/ "Fzul's Blog — What is MB Loader and how to use it?"
