...
| **Bedrock PBR** | Un `.mcpack` | Color, `_normal`, `_mer` y `*.texture_set.json` | Bedrock RTX y Vibrant Visuals. |
...
| **Bedrock PBR** | Un `.mcpack` propio o el ZIP completo del Vanilla Resource Pack de Bedrock. | Imágenes individuales, APK y `.jar`. |

1. Elige **Java 1.16+** o **Bedrock PBR**; la zona de carga solo mostrará formatos compatibles con ese destino.
2. Para Java, arrastra uno o varios ZIP/JAR. Para Bedrock, usa un MCPACK o descarga el **[Vanilla Resource Pack oficial][7]** y sube su ZIP sin extraerlo.
3. Selecciona un preset Glossy y revisa el preview permanente **Antes / Después**.
4. Pulsa convertir. Cada archivo produce una salida independiente: ZIP para Java o MCPACK para Bedrock.
